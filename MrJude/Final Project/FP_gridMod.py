...
#this is like the alien invasion to determine how much alien can fit in the row
...
class Pixel():
    def __init__(self, i_col,i_row, width, height, columns, rows, startx=0, starty=0, showGrid=False):
        ...
        self.hr = height / rows 
        #Use to display pixels inside the grid
    ...
    def show(self, screen, color, outline=False, showGrid=False): #Display the current pixel
        if not(showGrid): #For displaying color (pixel) inside the color pallet
            self.color = color
            pygame.draw.rect(screen, color, (self.x, self.y, self.wc, self.hr))
        
        if self.showGrid and not(outline): #For displaying outline in the color pallet 
